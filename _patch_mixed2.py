#!/usr/bin/env python3
"""Patch part 2: constants, plot, main loop"""
import sys
...
if old1 in raw:
    ...
    sys.exit("FAIL 1 - old1 not found")

# 5. reward_step_curve: replace single-plot with 3-panel plot
old5 = (
...
if old5 in raw:
    # First find the end of the old single-plot block to replace it whole
    ...
    new5_full = new5_start + (
        ...
        b"                for i in range(1, len(gm)):\r\n"
        b"                    ema_g[i] = EMA_ALPHA * gm[i] + (1 - EMA_ALPHA) * ema_g[i - 1]\r\n"
        b"                ax.plot(common_steps, ema_g, color=mode_colors.get(mg, 'black'),\r\n"
        b"                        linewidth=2.8, label=f'\xc3\x81tlag (n={len(interp_rows)} run)', zorder=10)\r\n"
        b"\r\n"
        b"        sm = plt.cm.ScalarMappable(cmap=cmap,\r\n"
        b"                                   norm=plt.Normalize(vmin=min(flow_levels), vmax=max(flow_levels)))\r\n"
        b"        sm.set_array([])\r\n"
        ...
        b"                     color=mode_colors.get(mg, 'black'), fontweight='bold')\r\n"
        b"        ax.set_ylim(-0.02, 1.02)\r\n"
    ...
    if old5_full in raw:
        raw = raw.replace(old5_full, new5_full, 1); changes.append("5:OK")
    else:
        sys.exit("FAIL 5 - old5_full not found in raw")
elif b"mode_group" in raw:
    changes.append("5:SKIP")
else:
    sys.exit("FAIL 5 - neither old5 nor mode_group found")
...
if old6 in raw:
    ...
    sys.exit("FAIL 6 - old6 not found")
