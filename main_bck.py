...
import csv
import itertools
import json  # --- ÚJ: JSON kezelés a mentéshez
import os  # --- ÚJ: Fájlműveletekhez
...
class SumoInternalParser:
    def __init__(self, file_path):
        ...
        self.file_path = file_path  # --- ÚJ: Eltároljuk az útvonalat
...
class PhaseDesignDialog:
        ...
        for i in range(self.n):
            for j in range(self.n):
                if i == j:
                    ...
                    cell_colors[i][j] = "#ffcccc"
                    cell_text[i][j] = "X"
                else:
                    cell_colors[i][j] = "#ccffcc"

        headers = [str(i + 1) for i in range(self.n)]
        self.table_obj = self.ax_matrix.table(
            cellText=cell_text, cellColours=cell_colors,
            rowLabels=headers, colLabels=headers,
            loc='center', cellLoc='center', bbox=[0, 0, 1, 1]
        )
        self.table_obj.auto_set_font_size(False)
        self.table_obj.set_fontsize(8)
            ...
            for (row, col), cell in self.table_obj.get_celld().items():
                if cell.contains(event)[0]:
                    ...
                    if 0 <= r < self.n and 0 <= c < self.n and r != c:
                        ns = not self.conflict_matrix[r, c]
                        self.conflict_matrix[r, c] = ns
                        self.conflict_matrix[c, r] = ns
                        self.draw_conflict_matrix()
                    return
    ...
    def get_arrow_type(self, idx):
        ...
        p1s, p2s = line['start_vec'];
        p1e, p2e = line['end_vec']
        as_ = math.atan2(p2s[1] - p1s[1], p2s[0] - p1s[0]);
        ae_ = math.atan2(p2e[1] - p1e[1], p2e[0] - p1e[0])
        diff = math.degrees(ae_ - as_)
    ...
    def draw_phase_icon(self, ax, indices):
        ...
        for idx in indices:
            ...
            sy = -math.sin(rad) * 0.8
            ex, ey = 0, 0;
            style = "Simple, tail_width=1.0, head_width=5, head_length=5";
            conn = "arc3,rad=0"
            if atype == 'straight':
                ex = math.cos(rad) * 0.8;
                ey = math.sin(rad) * 0.8
                ...
                erad = rad + math.pi / 2;
                ex = math.cos(erad) * 0.8;
                ey = math.sin(erad) * 0.8;
                conn = "arc3,rad=-0.4"
            else:
                erad = rad - math.pi / 2;
                ex = math.cos(erad) * 0.8;
                ey = math.sin(erad) * 0.8;
                conn = "arc3,rad=0.4"
            arrow = FancyArrowPatch((sx, sy), (ex, ey), connectionstyle=conn, arrowstyle=style,
    ...
    def generate_phases(self):
        ...
        for i in range(self.n):
            for j in range(i + 1, self.n):
                if not self.conflict_matrix[i, j]: G.add_edge(i, j)
        cliques = list(nx.find_cliques(G));
        cliques.sort(key=len, reverse=True)
        for w in self.result_frame.winfo_children(): w.destroy()
        for i, clique in enumerate(cliques):
            if not clique: continue
            f = tk.LabelFrame(self.result_frame, text=f"Fázis {i + 1}", font=("Arial", 10, "bold"),
                              pady=5, padx=5)
            f.pack(fill=tk.X, padx=5, pady=5)
            fig = Figure(figsize=(2, 2), dpi=100);
            ax = fig.add_subplot(111)
            self.draw_phase_icon(ax, sorted(clique))
            canvas = FigureCanvasTkAgg(fig, master=f);
            canvas.get_tk_widget().pack(side=tk.LEFT)
            tk.Label(f, text="\n".join(
                [f"{x + 1}. {self.data['lines'][x]['id']}" for x in sorted(clique)]),
                     justify=tk.LEFT, font=("Consolas", 9)).pack(side=tk.LEFT, padx=10)
...
class JunctionApp:
    def __init__(self, root, parser):
        ...
        # --- ÚJ: Mentési fájl meghatározása és betöltés ---
        ...
        ttk.Button(top_frame, text="Beáll. mentés", command=self.save_settings).pack(
            side=tk.RIGHT, padx=10)
    ...
    def save_settings(self):
        if self.current_data_cache and self.matrix_mask is not None:
            jid = self.current_data_cache['jdata']['id']
            self.saved_settings[jid] = {'matrix_mask': self.matrix_mask.tolist()}
            try:
                with open(self.settings_file, 'w', encoding='utf-8') as f:
                    json.dump(self.saved_settings, f, indent=4)
                messagebox.showinfo("Mentés", "Beállítások mentve.")
            except Exception as e:
    ...
    def plot_current(self):
        ...
        # Megnézzük, van-e mentett maszk ehhez a csomóponthoz
        current_id = data['jdata']['id']
        if current_id in self.saved_settings and 'matrix_mask' in self.saved_settings[current_id]:
            # Ha van, és a mérete egyezik a jelenlegi vonalak számával, akkor betöltjük
            loaded_mask = np.array(self.saved_settings[current_id]['matrix_mask'])
            if loaded_mask.shape == (len(data['lines']), len(data['lines'])):
                self.matrix_mask = loaded_mask
            else:
                # Ha a méret nem stimmel (pl. változott a geometria), akkor nullázzuk
                self.matrix_mask = None
        else:
            self.matrix_mask = None  # Nincs mentés, az update_tables fogja inicializálni

        # ... (A plot_current többi része változatlan) ...
        # Háttér
        if 'shape' in data['jdata'] and data['jdata']['shape']:
            self.ax.add_patch(
                MplPolygon(data['jdata']['shape'], facecolor='#e6e6e6', edgecolor='#999999',
